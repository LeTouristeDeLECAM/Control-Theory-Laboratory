...
def FF_RT (DV , thetad, thetap, Ts, MVFFDelay, KFF, T1p, T1d, MVFFLL1, T2p, T2d, MV_FF, DV0=0):
    ...
    DV : Disturbance
    thetad : delay of the disturbance
    thetap : delay of the process
    Ts : Sampling time
    MVFFDelay : array
    KFF : -kp/Kc
    T1p : first time constant of process
    T1d : first time constant of disturbance
    MVFFLL1 : array
    T2p : second time constant of process
    T2d : second time constant of disturbance
    MV_FF : array
    """
...
#---------------------------------------------------------------------  
def Display_Bode(Broida1,Broida2,Strejc,NumFOPDT,NumSOPDT, omega):
    """
    :Broida1: Process object
    :Broida2: Process object
    :Strejc: Process object
    :NumFOPDT: Process object
    :NumSOPDT: Process object
    :omega: frequency vector (rad/s); generated by a command of the type "omega = np.logspace(-2, 2, 10000)".
    This function generates the Bode diagram of serval process models
    """
    fig, (ax_gain, ax_phase) = plt.subplots(2,1)
    fig.set_figheight(12)    
    fig.set_figwidth(22)
    ...
    # Gain part
    ax_gain.semilogx(omega,20*np.log10(np.abs(Broida1)),label='Broida1')
    ax_gain.semilogx(omega,20*np.log10(np.abs(Broida2)),label='Broida2')
    ax_gain.semilogx(omega,20*np.log10(np.abs(Strejc)),label='Strejc')
    ax_gain.semilogx(omega,20*np.log10(np.abs(NumFOPDT)),label='NumFOPDT')
    ax_gain.semilogx(omega,20*np.log10(np.abs(NumSOPDT)),label='NumSOPDT')

    gain_min = np.min([np.min(20*np.log10(np.abs(Broida1)/5)),np.min(20*np.log10(np.abs(Broida2)/5)),np.min(20*np.log10(np.abs(Strejc)/5)),np.min(20*np.log10(np.abs(NumFOPDT)/5)),np.min(20*np.log10(np.abs(NumSOPDT)/5))])
    gain_max = np.max([np.max(20*np.log10(np.abs(Broida1)*5)),np.max(20*np.log10(np.abs(Broida2)*5)),np.max(20*np.log10(np.abs(Strejc)*5)),np.max(20*np.log10(np.abs(NumFOPDT)*5)),np.max(20*np.log10(np.abs(NumSOPDT)*5))])
    ...
    # Phase part
    ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(Broida1)),label='Broida1')
    ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(Broida2)),label='Broida2')
    ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(Strejc)),label='Strejc')
    ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(NumFOPDT)),label='NumFOPDT')
    ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(NumSOPDT)),label='NumSOPDT')
    ax_phase.set_xlabel(r'Frequency $\omega$ [rad/s]')
     
    ax_phase.set_xlim([np.min(omega), np.max(omega)])
    ph_min = np.min([np.min((180/np.pi)*np.unwrap(np.angle(Broida1))) - 10,np.min((180/np.pi)*np.unwrap(np.angle(Broida2))) - 10,np.min((180/np.pi)*np.unwrap(np.angle(Strejc))) - 10,np.min((180/np.pi)*np.unwrap(np.angle(NumFOPDT))) - 10,np.min((180/np.pi)*np.unwrap(np.angle(NumSOPDT))) - 10])
    ph_max = np.max([np.max((180/np.pi)*np.unwrap(np.angle(Broida1))) + 10,np.max((180/np.pi)*np.unwrap(np.angle(Broida2))) + 10,np.max((180/np.pi)*np.unwrap(np.angle(Strejc))) + 10,np.max((180/np.pi)*np.unwrap(np.angle(NumFOPDT))) + 10,np.max((180/np.pi)*np.unwrap(np.angle(NumSOPDT))) + 10])
    ax_phase.set_ylim([np.max([ph_min, -200]), ph_max])
